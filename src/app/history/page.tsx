...
const ranges: { key: RangeKey; label: string; points: number; minutesStep: number }[] = [
...
export default function HistoryPage() {
  ...
  const options = useMemo(() => ({
    ...
    plugins: {
      legend: { position: "bottom" as const, labels: { color: "#334155" } },
  ...
  return (
    <main className="flex flex-col items-center w-full min-h-screen p-4 bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 sm:p-6 md:p-10">
      <header className="mt-8 mb-10 flex w-full max-w-7xl items-center justify-between">
        <div className="flex items-center gap-4">
          <Link 
            href="/" 
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/70 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 border border-white/30 dark:border-slate-700/50 hover:bg-white/90 dark:hover:bg-slate-700/70 transition-colors"
          >
            <ArrowLeft className="w-4 h-4" />
            Zurück
          </Link>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Verlauf</h1>
        </div>
            ...
            <button
              ...
              className={`px-3 py-2 rounded-xl text-sm font-medium border transition-colors ${
                range === r.key
                  ...
                  : "bg-white/70 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 border-white/30 dark:border-slate-700/50 hover:bg-white/90 dark:hover:bg-slate-700/70"
              }`}
            >
              {r.label}
            </button>
      ...
      <section className="w-full max-w-7xl grid grid-cols-1 gap-6">
        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/30 dark:border-slate-700/50 rounded-3xl p-6 shadow-lg" style={{ height: 420 }}>
